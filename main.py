...
def app():
    ...
    # 데이터 전처리: 행정구역 이름 정제
    # 행정구역 컬럼에는 "서울특별시  (1100000000)" 과 같이 행정구역 코드가
    # 괄호로 함께 표기되어 있다. 기존 구현은 공백 기준 첫 단어만 추출해
    # "전라남도 여수시" 같은 다단어 지역명을 올바르게 처리하지 못했다.
    # 괄호와 그 안의 내용을 제거하고 앞뒤 공백을 정리해 전체 지역명을 유지한다.
    df['행정구역'] = (
        df['행정구역']
        .astype(str)
